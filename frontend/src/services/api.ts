...
const getConfig = async (): Promise<{ API_BASE_URL: string }> => {
  if (typeof window !== 'undefined') {
    // If running on Vite dev port, assume backend is at localhost:8000
    if (window.location.port === '5173') {
       return { API_BASE_URL: 'http://localhost:8000' };
    }
    // For production/build, assume backend is serving us (same origin).
    // Returning empty string makes all fetch calls relative (e.g. /refresh).
    return { API_BASE_URL: '' };
  }
  return { API_BASE_URL: 'http://localhost:8000' };
};
